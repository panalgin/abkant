...
#define LCD_I2CADDR 0x26
#define KEYPAD_I2CADDR 0x27
...
Keypad_I2C keypad = Keypad_I2C( makeKeymap(keys), rowPins, colPins, ROWS, COLS, KEYPAD_I2CADDR );
...
const uint8_t pressureThresholdPin = A1;
...
void setup() {
  ...
  pinMode(pressureThresholdPin, INPUT);
...
uint16_t pressureThresholdValue = 650;

ISR(TIMER1_COMPA_vect) {
  uint16_t pressureValue = analogRead(pressureSensorPin);
  pressureThresholdValue = analogRead(pressureThresholdPin);

  uint16_t pressureInBar = map(pressureValue, 0, 1023, 0, 400);
  uint16_t pressureThresholdInBar = map(pressureThresholdValue, 0, 1023, 0, 400);

  if (pressureInBar >= pressureThresholdInBar) {
    ...
    if (hasPressureRelieved == true && millis() - lastPressuredAt > 3000) {
  ...
  if (m_Main.Type == 6) {
    if (m_Main.HasFiredMotor == false) {
      ...
      if (finalDestination >= minimumDestination) {
        x_Motor.SetMaxSpeed(125);
        x_Motor.SetDwellSpeed(25);
        x_Motor.SetSpeed(125);
